    ...
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            ...
            CREATE TABLE IF NOT EXISTS users (
                ...
                plan_started_at INTEGER DEFAULT 0,
                plan_expires_at INTEGER DEFAULT 0,
                plan_exempt INTEGER DEFAULT 0,
                plan_locked INTEGER DEFAULT 0,
                plan_expired_notified INTEGER DEFAULT 0,
        ...
        await _ensure_column(db, "users", "plan_started_at", "INTEGER DEFAULT 0")
        await _ensure_column(db, "users", "plan_expires_at", "INTEGER DEFAULT 0")
        await _ensure_column(db, "users", "plan_exempt", "INTEGER DEFAULT 0")
        await _ensure_column(db, "users", "plan_locked", "INTEGER DEFAULT 0")
        await _ensure_column(db, "users", "plan_expired_notified", "INTEGER DEFAULT 0")
        ...
        await _ensure_column(db, "users", "created_at", "INTEGER DEFAULT 0")
        await _ensure_column(db, "users", "last_seen_at", "INTEGER DEFAULT 0")
        await _ensure_column(db, "users", "last_login_at", "INTEGER DEFAULT 0")
        await _ensure_column(db, "users", "login_success_count", "INTEGER DEFAULT 0")
        await _ensure_column(db, "users", "login_failure_count", "INTEGER DEFAULT 0")
        await db.execute("UPDATE users SET created_at=strftime('%s','now') WHERE COALESCE(created_at,0)=0")
        ...
        await db.execute("UPDATE users SET plan_started_at = strftime('%s','now') WHERE COALESCE(plan_started_at,0)=0")
        ...
        await db.execute("INSERT OR IGNORE INTO system_settings(key, value) VALUES ('plan_validity_config', '{\"global_enabled\":true,\"default_days\":15}')")
        await db.execute("INSERT OR IGNORE INTO system_settings(key,value) VALUES ('registration_open','1')")
        await db.execute("INSERT OR IGNORE INTO system_settings(key,value) VALUES ('global_maintenance','0')")
        await db.execute("INSERT OR IGNORE INTO system_settings(key,value) VALUES ('anti_spam_protect','1')")
        await db.execute("INSERT OR IGNORE INTO system_settings(key,value) VALUES ('log_deleted_msgs','1')")
        await db.execute("INSERT OR IGNORE INTO system_settings(key,value) VALUES ('max_allowed_selfs','1000')")
        await db.execute("INSERT OR IGNORE INTO system_settings(key,value) VALUES ('support_channel_url','https://t.me/Vip_Viro')")
