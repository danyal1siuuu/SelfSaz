...
from core.plan_control import user_plan_state, enforce_expired_clients
import aiosqlite
...
def clean_profile_name(first_name: str) -> str:
    ...
    if len(parts) == 2:
        tail = parts[1].strip()
        allowed_tail = all_time_chars | set("[]〔〕【】⟦⟧⟪⟫‹›《》•✦◆◈⏱⌚—·")
        normalized = tail.strip("[]〔〕【】⟦⟧⟪⟫‹›《》•✦◆◈⏱⌚—· ")
        if ":" in normalized and normalized and all(c in allowed_tail for c in tail):
    ...
    while getattr(client, "timename_active", False):
        try:
            ...
            if now_t != last_t:
                ...
                rendered = CLOCK_TEMPLATES.get(int(font_id), "{t}").format(t=clock_str)
                await client.update_profile(first_name=f"{base_name} {rendered}"[:64])
        except Exception:
            pass
        try:
...
async def start_single_client(user_id: int, session_str: str):
    ...
    if state.get("expired") and not state.get("exempt"):
        return False, "اعتبار پلن شما تمام شده است؛ ابتدا از بخش رنک/ارتقا استفاده کنید."
    ...
    try:
        ...
        cli.original_bio = settings.get("original_bio") if settings.get("original_bio") is not None else (getattr(me, "bio", "") or "")
        settings["original_bio"] = cli.original_bio
        cli.fixed_bio_enabled = bool(settings.get("fixed_bio_enabled", False))
        cli.fixed_bio_text = str(settings.get("fixed_bio_text") or "SelfSaz • آنلاین 🟢")[:70]
        ...
        try:
            ...
            saved_font = 1
        cli.timename_font = min(40, max(1, saved_font))
        ...
        if cli.fixed_bio_enabled:
            try:
                await cli.update_profile(bio=cli.fixed_bio_text)
            except Exception:
                pass
...
async def stop_single_client(user_id: int):
    if user_id in ACTIVE_CLIENTS:
        try:
            ...
            if getattr(cli, "fixed_bio_enabled", False) or getattr(cli, "original_bio", None) is not None:
                try:
...
async def plan_expiry_watchdog():
    """Continuously stop self accounts whose plan has just expired."""
    while True:
        try:
            stopped = await enforce_expired_clients(ACTIVE_CLIENTS)
            if stopped:
                print(f"[⏳ Plan] {stopped} expired self account(s) stopped.", flush=True)
        except asyncio.CancelledError:
            raise
        except Exception as exc:
            print(f"[⏳ Plan] watchdog error: {type(exc).__name__}: {exc}", flush=True)
