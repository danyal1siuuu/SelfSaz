...
PLANS_DATA: dict[str, dict[str, Any]] = {
    ...
    "iron": {
        ...
        "price_coins": 100, "required_referrals": 1,
    ...
    "bronze": {
        ...
        "price_coins": 250, "required_referrals": 3,
    ...
    "silver": {
        ...
        "price_coins": 500, "required_referrals": 6,
    ...
    "gold": {
        ...
        "price_coins": 1000, "required_referrals": 10,
    ...
    "diamond": {
        ...
        "price_coins": 2000, "required_referrals": 20,
...
def render_plans_catalog(profile: dict[str, Any]) -> str:
    ...
    lines = ["🛍 **فروشگاه رنک‌های سلف‌ساز**", "━━━━━━━━━━━━━━━━━━━━", "برای ارتقا: سکه کافی + تعداد دعوت موردنیاز همان رنک.", ""]
...
async def handle_plan_callback(bot, cq: dict[str, Any], *, user_id: int, chat_id: int, msg_id: int, data: str) -> bool:
    if data not in {"menu_plans", "plan_catalog", "plan_claim_daily", "plan_referral", "plan_wallet"} and not data.startswith("plan_buy_"):
        return False
    profile = await get_user_profile(user_id)
    if not profile:
    ...
    if data == "plan_referral":
        ...
        await bot.answer_callback(cq["id"])
        await bot.edit_message(chat_id, msg_id, f"👥 **دعوت دوستان**\n\nلینک اختصاصی شما:\n`{link}`\n\n🎁 با اولین ثبت‌نام موفق دوست دعوت‌شده، `{50}` سکه و `25 XP` دریافت می‌کنی.\n📊 دعوت‌های موفق شما: `{profile['referral_count']}`", reply_markup={"inline_keyboard": [[{"text": "🪙 کیف پول", "callback_data": "plan_wallet"}], [{"text": "🔙 بازگشت", "callback_data": "menu_plans"}]]})
        return True
    if data == "plan_wallet":
        cfg = get_plan_config(profile["plan"])
        await bot.answer_callback(cq["id"])
