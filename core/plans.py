...
from core.plan_control import apply_plan_expiry, user_plan_state, get_plan_control
...
PLANS_DATA: dict[str, dict[str, Any]] = {
    "normal": {
        ...
        "max_friends": 3, "max_enemies": 3, "allowed_fonts": list(range(1, 11)),
        "min_cleaner_delay": 60, "ai_monshi": False, "anti_delete_logger": False,
        "daily_coins": 5, "activity_coins": 1, "activity_daily_cap": 20,
        "max_yt_mb": 100, "max_notes": 10, "max_ai_memory": 20, "max_auto_reply_chars": 500, "timename_interval": 60, "daily_yt_downloads": 2, "automation_react_daily": 0, "automation_save_daily": 5, "automation_greet_daily": 2, "automation_name_count": 2, "automation_bio_count": 2, "automation_cycle_interval": 300, "automation_typing_delay": 0.8, "description": "شروع رایگان برای استفاده‌های پایه",
    },
    "iron": {
        "title": "آهنی", "short_title": "آهنی", "badge": "⛓️",
        "price_coins": 500, "required_referrals": 5,
        "max_friends": 5, "max_enemies": 5, "allowed_fonts": list(range(1, 17)),
        "min_cleaner_delay": 30, "ai_monshi": False, "anti_delete_logger": True,
        "daily_coins": 10, "activity_coins": 2, "activity_daily_cap": 40,
        "max_yt_mb": 250, "max_notes": 25, "max_ai_memory": 50, "max_auto_reply_chars": 800, "timename_interval": 50, "daily_yt_downloads": 5, "automation_react_daily": 10, "automation_save_daily": 20, "automation_greet_daily": 5, "automation_name_count": 3, "automation_bio_count": 3, "automation_cycle_interval": 240, "automation_typing_delay": 0.7, "description": "پایه تقویت‌شده با امکانات بیشتر",
    },
    "bronze": {
        "title": "برنزی", "short_title": "برنزی", "badge": "🥉",
        "price_coins": 1500, "required_referrals": 15,
        "max_friends": 15, "max_enemies": 15, "allowed_fonts": list(range(1, 25)),
        "min_cleaner_delay": 20, "ai_monshi": False, "anti_delete_logger": True,
        "daily_coins": 25, "activity_coins": 3, "activity_daily_cap": 80,
        "max_yt_mb": 500, "max_notes": 50, "max_ai_memory": 100, "max_auto_reply_chars": 1200, "timename_interval": 40, "daily_yt_downloads": 10, "automation_react_daily": 30, "automation_save_daily": 50, "automation_greet_daily": 10, "automation_name_count": 4, "automation_bio_count": 4, "automation_cycle_interval": 180, "automation_typing_delay": 0.5, "description": "برای کاربران فعال با سقف‌های بالاتر",
    },
    "silver": {
        "title": "نقره‌ای", "short_title": "نقره‌ای", "badge": "🥈",
        "price_coins": 4000, "required_referrals": 30,
        "max_friends": 40, "max_enemies": 40, "allowed_fonts": list(range(1, 33)),
        "min_cleaner_delay": 10, "ai_monshi": True, "anti_delete_logger": True,
        "daily_coins": 50, "activity_coins": 4, "activity_daily_cap": 150,
        "max_yt_mb": 1024, "max_notes": 100, "max_ai_memory": 250, "max_auto_reply_chars": 1800, "timename_interval": 30, "daily_yt_downloads": 20, "automation_react_daily": 60, "automation_save_daily": 100, "automation_greet_daily": 20, "automation_name_count": 6, "automation_bio_count": 6, "automation_cycle_interval": 120, "automation_typing_delay": 0.4, "description": "سطح نیمه‌حرفه‌ای با منشی هوشمند",
    },
    "gold": {
        "title": "طلایی", "short_title": "طلایی", "badge": "🥇",
        "price_coins": 10000, "required_referrals": 60,
        "max_friends": 100, "max_enemies": 100, "allowed_fonts": list(range(1, 41)),
        "min_cleaner_delay": 5, "ai_monshi": True, "anti_delete_logger": True,
        "daily_coins": 100, "activity_coins": 6, "activity_daily_cap": 300,
        "max_yt_mb": 2048, "max_notes": 250, "max_ai_memory": 500, "max_auto_reply_chars": 2600, "timename_interval": 20, "daily_yt_downloads": 50, "automation_react_daily": 120, "automation_save_daily": 250, "automation_greet_daily": 50, "automation_name_count": 8, "automation_bio_count": 8, "automation_cycle_interval": 90, "automation_typing_delay": 0.3, "description": "سطح حرفه‌ای برای استفاده سنگین",
    },
    "diamond": {
        "title": "الماسی", "short_title": "الماسی", "badge": "💎",
        "price_coins": 25000, "required_referrals": 120,
        "max_friends": 9999, "max_enemies": 9999, "allowed_fonts": list(range(1, 41)),
        "min_cleaner_delay": 1, "ai_monshi": True, "anti_delete_logger": True,
        "daily_coins": 250, "activity_coins": 10, "activity_daily_cap": 600,
        "max_yt_mb": None, "max_notes": 1000, "max_ai_memory": 1000, "max_auto_reply_chars": 3500, "timename_interval": 10, "daily_yt_downloads": 1000, "automation_react_daily": 300, "automation_save_daily": 500, "automation_greet_daily": 100, "automation_name_count": 12, "automation_bio_count": 12, "automation_cycle_interval": 60, "automation_typing_delay": 0.2, "description": "بالاترین سطح؛ بدون سقف پلن برای حجم یوتیوب",
    },
}
...
async def get_user_profile(user_id: int) -> dict[str, Any] | None:
    async with aiosqlite.connect(DB_NAME) as db:
        cur = await db.execute(
            """SELECT coins, plan, activity_score, last_daily_claim, is_vip,
                      ...
                      activity_reward_date, activity_reward_coins, plan_started_at,
    ...
    profile = {
        ...
        "plan_started_at": int(row[11] or 0),
        "plan_expires_at": int(row[12] or 0),
        "plan_exempt": bool(row[13] or 0),
        "plan_locked": bool(row[14] or 0),
        "plan_expired_notified": bool(row[15] or 0),
    }
    if not profile["plan_started_at"]:
        await apply_plan_expiry(user_id, profile["plan"], reset_start=True)
        profile["plan_started_at"] = int(time.time())
        refreshed = await user_plan_state(user_id)
        profile["plan_expires_at"] = int(refreshed.get("expires_at") or 0)
    elif not profile["plan_expires_at"]:
        cfg = await get_plan_control()
        rule = cfg["plans"].get(profile["plan"], {})
        if cfg.get("global_enabled", True) and rule.get("enabled", False) and int(rule.get("days", 0) or 0) > 0 and not profile["plan_exempt"]:
            await apply_plan_expiry(user_id, profile["plan"], reset_start=False)
            refreshed = await user_plan_state(user_id)
            profile["plan_expires_at"] = int(refreshed.get("expires_at") or 0)
    return profile
...
async def get_plan_limit(user_id: int, key: str, default: int = 0) -> int:
    profile = await get_user_profile(user_id)
    cfg = get_plan_config(profile["plan"] if profile else "normal")
    try:
...
async def get_notes_limit(user_id: int) -> int:
    return await get_plan_limit(user_id, "max_notes", 10)

async def get_ai_memory_limit(user_id: int) -> int:
    return await get_plan_limit(user_id, "max_ai_memory", 20)

async def get_auto_reply_limit(user_id: int) -> int:
    return await get_plan_limit(user_id, "max_auto_reply_chars", 500)

async def get_timename_interval(user_id: int) -> int:
    return max(10, await get_plan_limit(user_id, "timename_interval", 60))

async def get_automation_limit(user_id: int, key: str, default: int = 0):
    return await get_plan_limit(user_id, key, default)

async def get_automation_react_limit(user_id: int) -> int:
    return await get_automation_limit(user_id, "automation_react_daily", 0)

async def get_automation_save_limit(user_id: int) -> int:
    return await get_automation_limit(user_id, "automation_save_daily", 5)

async def get_automation_greet_limit(user_id: int) -> int:
    return await get_automation_limit(user_id, "automation_greet_daily", 2)

async def get_automation_name_count(user_id: int) -> int:
    return await get_automation_limit(user_id, "automation_name_count", 2)

async def get_automation_bio_count(user_id: int) -> int:
    return await get_automation_limit(user_id, "automation_bio_count", 2)

async def get_automation_interval(user_id: int) -> int:
    return max(60, await get_automation_limit(user_id, "automation_cycle_interval", 300))

async def get_daily_yt_limit(user_id: int) -> int:
    return await get_plan_limit(user_id, "daily_yt_downloads", 2)

async def get_daily_yt_usage(user_id: int) -> tuple[int, int]:
    """Return (used_today, daily_limit) without consuming a slot."""
    limit = await get_daily_yt_limit(user_id)
    today = time.strftime("%Y-%m-%d", time.localtime())
    async with aiosqlite.connect(DB_NAME) as db:
        row = await (await db.execute("SELECT youtube_download_date, youtube_download_count FROM users WHERE user_id=?", (user_id,))).fetchone()
    used = int(row[1] or 0) if row and row[0] == today else 0
    return used, limit

async def record_daily_yt_download(user_id: int) -> tuple[int, int]:
    """Record one successful download and return (new_used, daily_limit)."""
    limit = await get_daily_yt_limit(user_id)
    today = time.strftime("%Y-%m-%d", time.localtime())
    async with aiosqlite.connect(DB_NAME) as db:
        row = await (await db.execute("SELECT youtube_download_date, youtube_download_count FROM users WHERE user_id=?", (user_id,))).fetchone()
        used = int(row[1] or 0) if row and row[0] == today else 0
        new_count = min(limit, used + 1)
        await db.execute("UPDATE users SET youtube_download_date=?, youtube_download_count=? WHERE user_id=?", (today, new_count, user_id))
        await db.commit()
...
async def upgrade_user_plan(user_id: int, target_plan: str) -> tuple[bool, str]:
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute("UPDATE users SET coins = coins - ?, plan = ?, is_vip = ?, previous_plan = ? WHERE user_id = ?", (price, target_plan, 1 if target_plan == "diamond" else 0, current, user_id))
        await db.commit()
    await apply_plan_expiry(user_id, target_plan, reset_start=True)
...
async def admin_set_plan(user_id: int, target_plan: str) -> tuple[bool, str]:
    ...
    if not profile:
        return False, "❌ کاربر یافت نشد."
    async with aiosqlite.connect(DB_NAME) as db:
        current = (await (await db.execute("SELECT plan FROM users WHERE user_id=?", (user_id,))).fetchone())[0] or "normal"
        await db.execute("UPDATE users SET plan = ?, is_vip = ?, previous_plan = ? WHERE user_id = ?", (target_plan, 1 if target_plan == "diamond" else 0, current, user_id))
        await db.commit()
    await apply_plan_expiry(user_id, target_plan, reset_start=True)
    cfg = PLANS_DATA[target_plan]
...
def render_plan_dashboard(profile: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"🎨 استایل ساعت: `{len(cfg['allowed_fonts'])}/40`\n"
