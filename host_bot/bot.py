...
class HttpBot:
    ...
    async def _finish_qr_login(self, user_id, chat_id):
        ...
        try:
            ...
            await self.send_message(chat_id, "🎉 **ورود با QR با موفقیت انجام شد.**\n\n🔐 سلف شما ساخته و روشن شد.\n\nاز داشبورد زیر مدیریت را ادامه دهید.", reply_markup=self.get_main_dashboard_kb(True, user_id == ADMIN_ID))
    ...
    async def handle_update(self, update):
        ...
        if "message" in update:
            ...
            # مسیر قدیمی ورود با شماره عمداً غیرفعال شده است.
            # کد ورود نباید داخل چت ربات ارسال شود؛ QR رسمی استفاده می‌شود.
            if USER_STATES.get(user_id) == "WAITING_PHONE":
                USER_STATES.pop(user_id, None)
                return await self.send_message(
                    chat_id,
                    "⛔ ورود با ارسال کد داخل چت غیرفعال شده است.\n\n"
                    "برای ورود، از «ورود امن با QR» استفاده کنید.",
                    reply_markup={"inline_keyboard": [[{"text": "🔐 ورود امن با QR", "callback_data": "btn_create_session"}]]},
                )

            # مرحله قدیمی کد ورود دیگر استفاده نمی‌شود.
            if USER_STATES.get(user_id) == "WAITING_LOGIN_CODE":
                USER_STATES.pop(user_id, None)
                data = LOGIN_CLIENTS.pop(user_id, None)
                if data:
                    try:
                        await data["client"].disconnect()
                    except Exception:
                ...
                return await self.send_message(
                    chat_id,
                    "⛔ کد ورود را داخل چت ربات ارسال نکنید. از «ورود امن با QR» استفاده کنید.",
                    reply_markup={"inline_keyboard": [[{"text": "🔐 ورود امن با QR", "callback_data": "btn_create_session"}]]},
                )
...
# پردازش کلیک روی دکمه‌ها
            ...
            if not is_admin and not data.startswith("ad_") and data not in allowed_unreg:
                ...
                if not u_chk:
                    ...
                    kb_reconnect = {"inline_keyboard": [[{"text": "🔐 ورود امن با QR", "callback_data": "btn_create_session"}, {"text": "🔑 ارسال سشن", "callback_data": "btn_submit_session"}]]}
            ...
            if data == "btn_create_session":
                ...
                kb = {"inline_keyboard": [[{"text": "🔐 ورود امن با QR", "callback_data": "btn_create_session"}, {"text": "🔑 ارسال سشن", "callback_data": "btn_submit_session"}]]}
