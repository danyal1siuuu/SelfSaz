...
class HttpBot:
    ...
    async def start(self):
        ...
        print("[+] Ultimate HTTP Bot Online with 115 Admin Features.")
        async with aiohttp.ClientSession() as session:
            while self.running:
                try:
    ...
    async def handle_update(self, update):
        ...
        if "message" in update:
            ...
            if text == "/admin" and is_admin:
                USER_STATES.pop(user_id, None)
                return await self.send_message(chat_id, "👑 **سوپر پنل اختصاصی مدیریت ادمین (۱۱۵ قابلیت مجزا)**\n\nیکی از بخش‌های زیر را انتخاب کنید:", reply_markup=self.get_admin_hub_kb())
            ...
            if USER_STATES.get(user_id) == "WAITING_PHONE":
                ...
                try:
                    ...
                    if getattr(sent, "next_type", None):
                        buttons.append([{"text": "📩 ارسال کد با روش بعدی", "callback_data": "btn_resend_login_code"}])
            ...
            if USER_STATES.get(user_id) == "WAITING_LOGIN_CODE":
                ...
                try:
                    ...
                    if "previously shared" in msg or "shared" in msg:
                        ...
                        return await self.send_message(chat_id, "⚠️ تلگرام این کد را به‌دلیل سیاست امنیتی ورود رد کرد. این محدودیت از سمت تلگرام است و با تغییر کد قابل دورزدن نیست.\n\n✅ روش QR را انتخاب کن تا ورود رسمی و مستقیم انجام شود.", reply_markup={"inline_keyboard": [[{"text": "🔐 ورود امن با QR", "callback_data": "btn_create_session"}], [{"text": "🔙 بازگشت", "callback_data": "back_dashboard"}]]})
...
# پردازش کلیک روی دکمه‌ها
            ...
            if data == "btn_phone_login":
                ...
                return await self.edit_message(
                    chat_id, msg_id,
                    "📱 **ورود با شماره تلفن**\n\nشماره حساب خودت را با فرمت بین‌المللی بفرست؛ مثال: `+989123456789`\n\n⚠️ کد ورود را فقط در همین گفت‌وگوی خصوصی با ربات وارد می‌کنی؛ اگر تلگرام کد را به‌خاطر اشتراک‌گذاری رد کند، سیستم به‌صورت خودکار مسیر QR را پیشنهاد می‌دهد.",
            ...
            # مرکز حرفه‌ای کاربر
                ...
                profile = await get_user_db(user_id)
                ...
                return await self.edit_message(chat_id, msg_id, text_pro, reply_markup=kb)
