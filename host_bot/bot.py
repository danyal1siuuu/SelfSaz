...
from core.plan_control import (
    user_plan_state, can_use_feature, expired_callback_allowed, build_admin_action_catalog,
    get_plan_control, set_plan_control_value, save_plan_control, format_expiry, PLAN_KEYS as VALIDITY_PLAN_KEYS, PLAN_LABELS as VALIDITY_PLAN_LABELS,
)
from core.recovery import export_user_backup, restore_user_backup
...
def add_system_log(text: str):
    ...
    if len(SYSTEM_LOGS) > 60:
        SYSTEM_LOGS.pop(0)
    # Railway captures stdout/stderr; keep operational errors visible there too.
...
class HttpBot:
            ...
            if cq:
                try:
                    await self.answer_callback(
                        cq.get("id", ""),
                        "❌ اجرای این دکمه با خطا متوقف شد. لاگ Railway را بررسی کنید.",
                        alert=True,
                    )
                except Exception:
                    pass
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status != 200 or not result.get("ok"):
                        add_system_log(f"Telegram sendMessage failed ({resp.status}): {result.get('description', result)}")
                    return result
        except Exception as e:
            add_system_log(f"Telegram sendMessage exception: {type(e).__name__}: {e}")
    ...
    async def edit_message(self, chat_id, message_id, text, reply_markup=None):
        ...
        try:
            ...
            if not result.get("ok"):
                add_system_log(f"Telegram editMessageText failed ({resp.status}): {result.get('description', result)}")
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    result = await resp.json(content_type=None)
                    if resp.status != 200 or not result.get("ok"):
                        add_system_log(f"Telegram answerCallbackQuery failed ({resp.status}): {result.get('description', result)}")
        except Exception as exc:
            add_system_log(f"Telegram answerCallbackQuery exception: {type(exc).__name__}: {exc}")
    ...
    async def copy_message(self, to_chat_id, from_chat_id, message_id, caption=None):
        payload = {"chat_id": to_chat_id, "from_chat_id": from_chat_id, "message_id": message_id}
        if caption is not None:
            payload["caption"] = caption
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{API_URL}/copyMessage", json=payload, timeout=aiohttp.ClientTimeout(total=30)) as resp:
                    result = await resp.json(content_type=None)
                    if not result.get("ok"):
                        add_system_log(f"Telegram copyMessage failed ({resp.status}): {result.get('description', result)}")
                    return result
        except Exception as exc:
            add_system_log(f"Telegram copyMessage exception: {type(exc).__name__}: {exc}")
    ...
    async def pin_chat_message(self, chat_id, message_id):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{API_URL}/pinChatMessage", json=payload, timeout=aiohttp.ClientTimeout(total=20)) as resp:
                    result = await resp.json(content_type=None)
                    if not result.get("ok"):
                        add_system_log(f"Telegram pinChatMessage failed ({resp.status}): {result.get('description', result)}")
                    return result
        except Exception as exc:
            add_system_log(f"Telegram pinChatMessage exception: {type(exc).__name__}: {exc}")
            return {"ok": False, "description": str(exc)}
    ...
    def get_main_dashboard_kb(self, is_online, is_admin=False):
        ...
        kb = [
            ...
            [{"text": "⏰ ساعت روی اسم (۴۰ استایل)", "callback_data": "menu_timename"}],
    ...
    def get_expired_dashboard_kb(self):
        return {"inline_keyboard": [
            [{"text": "👑 رنک‌ها و ارتقا", "callback_data": "menu_plans"}],
            [{"text": "👥 دعوت از دوستان", "callback_data": "menu_invite"}],
            [{"text": "📊 وضعیت اعتبار", "callback_data": "plan_validity_status"}],
            [{"text": "🔄 بررسی مجدد اعتبار", "callback_data": "plan_validity_refresh"}],
        ]}

    def get_recovery_kb(self):
        labels = [
            ("full", "🧰 پشتیبان کامل"), ("settings", "⚙️ تنظیمات"), ("relations", "🛡 دوستان و دشمنان"),
            ("notes", "📝 یادداشت‌ها"), ("automations", "⚙️ اتوماسیون‌ها"), ("ai", "🧠 حافظه AI"),
            ("stats", "📊 آمار پیام"), ("plan", "🏆 رنک و اقتصاد"), ("notifications", "🔔 اعلان‌ها"), ("session", "🔐 سشن"),
        ]
        rows = [[{"text": title + " · خروجی", "callback_data": f"recovery_export_{scope}"},
                 {"text": title + " · بازیابی", "callback_data": f"recovery_import_{scope}"}] for scope, title in labels]
        rows += [
            [{"text": "🔄 بررسی یکپارچگی پشتیبان", "callback_data": "recovery_validate"}],
            [{"text": "⚠️ راهنمای امنیت فایل", "callback_data": "recovery_security"}],
            [{"text": "🔙 مرکز کاربری", "callback_data": "menu_user_center"}],
        ]
        return {"inline_keyboard": rows}

    def get_admin_plan_validity_kb(self, page: int = 1):
        catalog = build_admin_action_catalog()
        per_page = 10
        page = max(1, min((len(catalog) + per_page - 1) // per_page, int(page)))
        items = catalog[(page - 1) * per_page: page * per_page]
        rows = [[{"text": f"{i + (page-1)*per_page + 1}. {item['label']}", "callback_data": f"apc_{i + (page-1)*per_page}"}] for i, item in enumerate(items)]
        nav = []
        total_pages = (len(catalog) + per_page - 1) // per_page
        if page > 1:
            nav.append({"text": "⬅️ قبلی", "callback_data": f"ad_plan_validity_page_{page-1}"})
        nav.append({"text": f"📚 صفحه {page}/{total_pages}", "callback_data": "ad_plan_validity_info"})
        if page < total_pages:
            nav.append({"text": "➡️ بعدی", "callback_data": f"ad_plan_validity_page_{page+1}"})
        if nav:
            rows.append(nav)
        rows.append([{"text": "🔙 هاب ادمین", "callback_data": "admin_hub"}])
        return {"inline_keyboard": rows}

    async def _recovery_download_file(self, file_id: str, filename: str) -> str:
        safe_name = os.path.basename(filename or "backup.json").replace(" ", "_")
        target = os.path.join("downloads", f"recovery_{int(time.time())}_{safe_name}")
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{API_URL}/getFile", json={"file_id": file_id}, timeout=aiohttp.ClientTimeout(total=20)) as resp:
                info = await resp.json(content_type=None)
        if not info.get("ok"):
            raise ValueError(info.get("description", "دریافت فایل از Telegram ناموفق بود."))
        file_path = info["result"]["file_path"]
        async with aiohttp.ClientSession() as session:
            async with session.get(f"https://api.telegram.org/file/bot{BOT_TOKEN}/{file_path}", timeout=aiohttp.ClientTimeout(total=30)) as resp:
                if resp.status != 200:
                    raise ValueError(f"HTTP {resp.status}")
                content = await resp.read()
        if len(content) > 8 * 1024 * 1024:
            raise ValueError("فایل پشتیبان بیش از سقف ۸MB است.")
        with open(target, "wb") as fh:
            fh.write(content)
        return target

    async def _send_user_backup(self, chat_id: int, user_id: int, scope: str):
        payload = await export_user_backup(user_id, scope)
        path = os.path.join("downloads", f"selfsaz_{scope}_backup_{user_id}_{int(time.time())}.json")
        with open(path, "w", encoding="utf-8") as fh:
            json.dump(payload, fh, ensure_ascii=False, indent=2)
        try:
            caption = f"♻️ پشتیبان {scope} حساب `{user_id}`\nنسخه: `{payload.get('version')}`"
            result = await self.send_document(chat_id, path, caption=caption)
            if isinstance(result, dict) and not result.get("ok", True):
                raise ValueError(result.get("description", "ارسال فایل ناموفق بود."))
        finally:
            try:
                os.remove(path)
            except OSError:
    ...
    async def _handle_recovery_document(self, chat_id: int, user_id: int, document: dict):
        state = USER_STATES.get(user_id, "")
        if state == "RECOVERY_VALIDATE":
            USER_STATES.pop(user_id, None)
            path = None
            try:
                filename = document.get("file_name") or "backup.json"
                path = await self._recovery_download_file(document.get("file_id", ""), filename)
                with open(path, "r", encoding="utf-8") as fh:
                    payload = json.load(fh)
                if payload.get("format") != "SelfSaz User Backup" or int(payload.get("user_id") or 0) != user_id:
                    raise ValueError("فایل معتبر نیست یا متعلق به این حساب نیست.")
                await self.send_message(chat_id, f"✅ فایل معتبر است.\n📦 نسخه: `{payload.get('version','?')}`\n🧩 بخش: `{payload.get('scope','full')}`\n📅 خروجی: `{payload.get('exported_at','-')}`")
                return True
            except Exception as exc:
                await self.send_message(chat_id, f"❌ اعتبارسنجی ناموفق بود:\n`{str(exc)[:700]}`")
                return True
            finally:
                if path:
                    try: os.remove(path)
                    except OSError: pass
        if state == "APC_SNAPSHOT_UPLOAD" and user_id == ADMIN_ID:
            USER_STATES.pop(user_id, None)
            path = None
            try:
                path = await self._recovery_download_file(document.get("file_id", ""), document.get("file_name") or "snapshot.json")
                with open(path, "r", encoding="utf-8") as fh:
                    cfg = json.load(fh)
                if not isinstance(cfg, dict) or "plans" not in cfg or "global_enabled" not in cfg:
                    raise ValueError("این فایل Snapshot اعتبار SelfSaz نیست.")
                await save_plan_control(cfg)
                await audit_log(user_id, "plan_snapshot_restore", detail="snapshot restored")
                await self.send_message(chat_id, "✅ Snapshot با موفقیت اعتبارسنجی و ذخیره شد.")
                return True
            except Exception as exc:
                add_system_log(f"Plan snapshot restore failed: {type(exc).__name__}: {exc}")
                await self.send_message(chat_id, f"❌ بازیابی Snapshot ناموفق بود:\n`{str(exc)[:700]}`")
                return True
            finally:
                if path:
                    try: os.remove(path)
                    except OSError: pass
        if not isinstance(state, str) or not state.startswith("RECOVERY_WAIT:"):
            return False
        scope = state.split(":", 1)[1]
        USER_STATES.pop(user_id, None)
        filename = document.get("file_name") or "backup.json"
        if not filename.lower().endswith(".json"):
            await self.send_message(chat_id, "❌ فقط فایل JSON پشتیبان SelfSaz قابل بازیابی است.")
            return True
        path = None
        try:
            path = await self._recovery_download_file(document.get("file_id", ""), filename)
            with open(path, "r", encoding="utf-8") as fh:
                payload = json.load(fh)
            if scope != "full" and payload.get("scope") not in {scope, "full"}:
                raise ValueError(f"این فایل برای بخش «{scope}» ساخته نشده است.")
            result = await restore_user_backup(user_id, payload, scope)
            # Sync the in-memory client after a settings restore; full/session restore reboots it.
            if result.get("session_restored"):
                ...
                if u and u[0]:
                    ...
                    if not ok:
                        raise ValueError(f"بازیابی شد ولی سلف دوباره روشن نشد: {err}")
            elif result.get("settings_restored") and user_id in ACTIVE_CLIENTS:
                u = await self.get_user_db(user_id)
                st = json.loads(u[5]) if u and u[5] else {}
                cli = ACTIVE_CLIENTS[user_id]
                cli.settings = st
                cli.custom_prefix = u[3] or "."
                cli.prefix_enabled = bool(u[4])
                cli.timename_font = min(40, max(1, int(st.get("timename_font", getattr(cli, "timename_font", 1)) or 1)))
                cli.timename_active = bool(st.get("timename_active", getattr(cli, "timename_active", False)))
                cli.fixed_bio_enabled = bool(st.get("fixed_bio_enabled", getattr(cli, "fixed_bio_enabled", False)))
                cli.fixed_bio_text = str(st.get("fixed_bio_text", getattr(cli, "fixed_bio_text", "SelfSaz • آنلاین 🟢")) or "SelfSaz • آنلاین 🟢")[:70]
            if "relations" in result.get("changed", []) and user_id in ACTIVE_CLIENTS:
                cli = ACTIVE_CLIENTS[user_id]
                async with aiosqlite.connect(DB_NAME) as db:
                    rels = await (await db.execute("SELECT target_id,type FROM relations WHERE owner_id=?", (user_id,))).fetchall()
                cli.friends_set.clear(); cli.enemies_set.clear()
                for target_id, rel_type in rels:
                    (cli.enemies_set if str(rel_type).lower() == "enemy" else cli.friends_set).add(int(target_id))
            async with aiosqlite.connect(DB_NAME) as db:
                await db.execute("UPDATE users SET plan_locked=0 WHERE user_id=? AND COALESCE(plan_exempt,0)=1", (user_id,))
                await db.commit()
            await audit_log(user_id, "user_backup_restore", detail=json.dumps(result, ensure_ascii=False)[:1000])
            await self.send_message(chat_id, "✅ بازیابی با موفقیت انجام شد.\nبخش‌های تغییرکرده: " + ", ".join(result.get("changed", [])))
            return True
        except Exception as exc:
            add_system_log(f"Recovery restore failed for {user_id}: {type(exc).__name__}: {exc}")
            await self.send_message(chat_id, f"❌ بازیابی ناموفق بود:\n`{str(exc)[:900]}`")
            return True
        finally:
            if path:
                try:
    ...
    async def _admin_plan_action(self, action_id: int, chat_id: int, user_id: int, msg_id: int, cq_id: str):
        catalog = build_admin_action_catalog()
        if action_id < 0 or action_id >= len(catalog):
            return await self.answer_callback(cq_id, "❌ قابلیت اعتبار نامعتبر است.", alert=True)
        item = catalog[action_id]
        kind, arg = item["kind"], item.get("arg")
        cfg = await get_plan_control()
        # Toggle / simple controls
        if kind == "global_toggle":
            key = str(arg); new = not bool(cfg.get(key, False)); cfg[key] = new; await set_plan_control_value(key, new)
            await audit_log(user_id, "plan_validity_toggle", detail=f"{key}={new}")
            return await self.answer_callback(cq_id, f"✅ {key}: {'فعال' if new else 'خاموش'}", alert=True)
        if kind == "global_int":
            USER_STATES[user_id] = f"APC_INPUT:{action_id}"
            await self.answer_callback(cq_id, "✏️ مقدار را ارسال کنید.")
            return await self.send_message(chat_id, f"✏️ مقدار عددی برای «{item['label']}» را ارسال کنید:")
        if kind == "notify_days_input":
            USER_STATES[user_id] = f"APC_INPUT:{action_id}"
            return await self.send_message(chat_id, "✏️ روزهای اعلان را مثل `7,3,1` ارسال کنید:")
        if kind == "cycle_mode" or kind == "rule_version":
            modes = ["rank_referral_only", "full_lock", "grace_only"] if kind == "cycle_mode" else None
            if modes:
                current = cfg.get(arg, modes[0]); nxt = modes[(modes.index(current) + 1) % len(modes)] if current in modes else modes[0]
                await set_plan_control_value(arg, nxt)
                return await self.answer_callback(cq_id, f"✅ سیاست به `{nxt}` تغییر کرد.", alert=True)
            return await self.answer_callback(cq_id, f"🧩 نسخه قوانین: `{cfg.get(arg, 1)}`", alert=True)
        if kind == "plan_toggle":
            pk, field = arg; rule = await self._update_plan_rule(pk, field, not bool(cfg["plans"][pk].get(field, False)))
            return await self.answer_callback(cq_id, f"✅ {VALIDITY_PLAN_LABELS[pk]} / {field}: {'فعال' if rule[field] else 'خاموش'}", alert=True)
        if kind == "plan_int":
            USER_STATES[user_id] = f"APC_INPUT:{action_id}"
            await self.answer_callback(cq_id, "✏️ مقدار را ارسال کنید.")
            return await self.send_message(chat_id, f"✏️ مقدار عددی برای «{item['label']}» را ارسال کنید:")
        if kind == "plan_mode":
            pk, field = arg; modes = ["registration", "assignment", "first_use"] if field == "start_policy" else ["rank_referral_only", "full_lock", "grace_only"]
            current = cfg["plans"][pk].get(field, modes[0]); nxt = modes[(modes.index(current)+1)%len(modes)] if current in modes else modes[0]
            await self._update_plan_rule(pk, field, nxt)
            return await self.answer_callback(cq_id, f"✅ {VALIDITY_PLAN_LABELS[pk]} / {field}: `{nxt}`", alert=True)
        if kind in {"plan_report", "plan_apply_all", "plan_reset_all"}:
            pk = arg[0]
            async with aiosqlite.connect(DB_NAME) as db:
                if kind == "plan_report":
                    n = (await (await db.execute("SELECT COUNT(*) FROM users WHERE plan=? AND plan_expires_at>0 AND plan_expires_at<=?", (pk, int(time.time())))).fetchone())[0]
                    return await self.answer_callback(cq_id, f"📊 {VALIDITY_PLAN_LABELS[pk]} منقضی: {n}", alert=True)
                if kind == "plan_apply_all":
                    rows = await (await db.execute("SELECT user_id FROM users WHERE plan=?", (pk,))).fetchall()
                else:
                    rows = await (await db.execute("SELECT user_id FROM users WHERE plan=?", (pk,))).fetchall()
            from core.plan_control import apply_plan_expiry
            for (uid,) in rows:
                await apply_plan_expiry(int(uid), pk, reset_start=True)
            return await self.answer_callback(cq_id, f"✅ روی {len(rows)} کاربر {VALIDITY_PLAN_LABELS[pk]} اعمال شد.", alert=True)
        # Everything below is intentionally implemented as real, persistent operations.
        return await self._execute_admin_plan_action(item, cfg, chat_id, user_id, cq_id)

    async def _update_plan_rule(self, plan_key: str, field: str, value: Any):
        cfg = await get_plan_control(); cfg["plans"][plan_key][field] = value; await set_plan_control_value(f"plans.{plan_key}.{field}", value); return cfg["plans"][plan_key]

    async def _execute_admin_plan_action(self, item: dict[str, Any], cfg: dict[str, Any], chat_id: int, admin_id: int, cq_id: str):
        kind = item["kind"]
        # Prompt-based controls. Action id is encoded by the state.
        if kind.endswith("_user") or kind in {"user_report","user_set_days","user_extend_days","user_expire_now","user_unexpire","user_exempt_toggle","user_reset_timer","user_set_expiry","user_lock_toggle","user_reapply_plan"}:
            USER_STATES[admin_id] = f"APC_INPUT:{build_admin_action_catalog().index(item)}"
            return await self.send_message(chat_id, "👤 آیدی عددی کاربر را بفرستید؛ در گزینه‌های تمدید/تنظیم تاریخ، مقدار را هم با فاصله بعد از آیدی بنویسید.")
        if kind.startswith("bulk_"):
            return await self._execute_bulk_action(kind, chat_id, admin_id, cq_id)
        if kind.startswith("broadcast_") or kind in {"dm_user","set_expiry_notice","set_expired_notice","set_extension_notice","set_upgrade_notice","set_lock_notice"}:
            USER_STATES[admin_id] = f"APC_INPUT:{build_admin_action_catalog().index(item)}"
            return await self.send_message(chat_id, "✏️ مقدار/متن مورد نظر را ارسال کنید.")
        if kind.startswith("report_") or kind in {"audit_report","audit_count","last_plan_changes","last_extensions","last_expiries","last_locks","last_exempts","self_test","sync_locks","repair_missing_timers","repair_invalid_expiry","repair_locked_flags","clear_notice_flags","show_config"}:
            return await self._execute_report_action(kind, cfg, chat_id, admin_id, cq_id)
        if kind in {"toggle_audit","audit_export","audit_cleanup","reset_config","snapshot_config","restore_snapshot","preview_expiry_notice"}:
            return await self._execute_control_action(kind, cfg, chat_id, admin_id, cq_id)
        return await self.answer_callback(cq_id, "✅ عملیات انجام شد.", alert=True)

    async def _execute_bulk_action(self, kind: str, chat_id: int, admin_id: int, cq_id: str):
        from core.plan_control import apply_plan_expiry
        now = int(time.time()); changed = 0
        async with aiosqlite.connect(DB_NAME) as db:
            if kind == "bulk_reset_trial":
                rows = await (await db.execute("SELECT user_id FROM users WHERE COALESCE(plan,'normal')='normal' AND (plan_started_at=0 OR plan_started_at IS NULL)" )).fetchall()
                for (uid,) in rows:
                    await apply_plan_expiry(int(uid), "normal", reset_start=True)
                changed = len(rows)
            elif kind == "bulk_lock_expired":
                cur = await db.execute("UPDATE users SET plan_locked=1 WHERE plan_expires_at>0 AND plan_expires_at<=? AND COALESCE(plan_exempt,0)=0", (now,)); changed = cur.rowcount
            elif kind == "bulk_unlock_all":
                cur = await db.execute("UPDATE users SET plan_locked=0 WHERE COALESCE(plan_locked,0)=1"); changed = cur.rowcount
            elif kind == "bulk_reset_normal":
                rows = await (await db.execute("SELECT user_id FROM users WHERE plan='normal'")).fetchall()
                for (uid,) in rows: await apply_plan_expiry(int(uid), "normal", reset_start=True)
                changed = len(rows)
            elif kind in {"bulk_extend_1","bulk_extend_7","bulk_extend_30"}:
                days = {"bulk_extend_1":1,"bulk_extend_7":7,"bulk_extend_30":30}[kind]
                cur = await db.execute("UPDATE users SET plan_expires_at=CASE WHEN plan_expires_at>0 THEN plan_expires_at+? ELSE ? END, plan_locked=0 WHERE plan_expires_at>0 AND plan_expires_at<=?", (days*86400, now+days*86400, now)); changed = cur.rowcount
            elif kind == "bulk_diamond_exempt":
                cur = await db.execute("UPDATE users SET plan_exempt=1,plan_locked=0 WHERE plan='diamond'"); changed = cur.rowcount
            elif kind == "bulk_diamond_unexpire":
                cur = await db.execute("UPDATE users SET plan_expires_at=0,plan_locked=0 WHERE plan='diamond'"); changed = cur.rowcount
            elif kind == "bulk_reset_notifications":
                cur = await db.execute("UPDATE users SET plan_expired_notified=0"); changed = cur.rowcount
            elif kind == "bulk_apply_rules":
                rows = await (await db.execute("SELECT user_id,plan FROM users")).fetchall()
                for uid, pk in rows: await apply_plan_expiry(int(uid), str(pk), reset_start=False)
                changed = len(rows)
        await audit_log(admin_id, "plan_validity_bulk", detail=f"{kind}={changed}")
        return await self.answer_callback(cq_id, f"✅ {kind}: {changed} رکورد تغییر کرد.", alert=True)

    async def _execute_report_action(self, kind: str, cfg: dict[str, Any], chat_id: int, admin_id: int, cq_id: str):
        now = int(time.time())
        async with aiosqlite.connect(DB_NAME) as db:
            if kind == "report_normal":
                n=await (await db.execute("SELECT COUNT(*) FROM users WHERE plan='normal' AND (plan_expires_at=0 OR plan_expires_at>?)",(now,))).fetchone(); return await self.answer_callback(cq_id,f"👤 عادی فعال: {n[0]}",alert=True)
            if kind == "report_expired":
                n=await (await db.execute("SELECT COUNT(*) FROM users WHERE plan_expires_at>0 AND plan_expires_at<=? AND COALESCE(plan_exempt,0)=0",(now,))).fetchone(); return await self.answer_callback(cq_id,f"🔒 منقضی: {n[0]}",alert=True)
            if kind in {"report_3d","report_7d"}:
                d=3 if kind=="report_3d" else 7; n=await (await db.execute("SELECT COUNT(*) FROM users WHERE plan_expires_at>? AND plan_expires_at<=?",(now,now+d*86400))).fetchone(); return await self.answer_callback(cq_id,f"⏳ تا {d} روز: {n[0]}",alert=True)
            if kind == "report_unlimited":
                n=await (await db.execute("SELECT COUNT(*) FROM users WHERE plan_expires_at=0 OR plan_exempt=1",)).fetchone(); return await self.answer_callback(cq_id,f"♾ بدون انقضا: {n[0]}",alert=True)
            if kind == "report_distribution":
                rows=await (await db.execute("SELECT plan,COUNT(*) FROM users GROUP BY plan ORDER BY plan")).fetchall(); return await self.send_message(chat_id,"📊 توزیع اعتبار\n"+"\n".join(f"• {VALIDITY_PLAN_LABELS.get(p,p)}: {n}" for p,n in rows))
            if kind == "report_avg_remaining":
                r=await (await db.execute("SELECT AVG(CASE WHEN plan_expires_at>? THEN plan_expires_at-? END) FROM users WHERE plan_expires_at>?",(now,now,now))).fetchone(); return await self.answer_callback(cq_id,f"⏱ میانگین: {(r[0] or 0)/86400:.2f} روز",alert=True)
            if kind == "report_upcoming":
                rows=await (await db.execute("SELECT user_id,plan,plan_expires_at FROM users WHERE plan_expires_at>? ORDER BY plan_expires_at LIMIT 10",(now,))).fetchall(); txt="⏳ ۱۰ انقضای نزدیک\n"+"\n".join(f"`{u}` · {VALIDITY_PLAN_LABELS.get(p,p)} · {format_expiry(e)}" for u,p,e in rows); return await self.send_message(chat_id,txt or "موردی نیست.")
            if kind in {"report_recent_expired","report_full"}:
                q="SELECT user_id,plan,plan_started_at,plan_expires_at,plan_locked,plan_exempt FROM users ORDER BY plan_expires_at ASC LIMIT 100" if kind=="report_full" else "SELECT user_id,plan,plan_started_at,plan_expires_at,plan_locked,plan_exempt FROM users WHERE plan_expires_at>0 AND plan_expires_at<=? ORDER BY plan_expires_at DESC LIMIT 10"
                rows=await (await db.execute(q,(now,) if kind=="report_recent_expired" else ())).fetchall(); txt="📋 گزارش اعتبار\n"+"\n".join(f"`{u}` · {VALIDITY_PLAN_LABELS.get(p,p)} · {format_expiry(e)} · lock={l} exempt={x}" for u,p,st,e,l,x in rows); return await self.send_message(chat_id,txt or "داده‌ای نیست.")
            if kind == "audit_count":
                n=await (await db.execute("SELECT COUNT(*) FROM audit_logs WHERE action LIKE 'plan_validity%' OR action LIKE 'user_backup_restore'")).fetchone(); return await self.answer_callback(cq_id,f"🧾 Audit اعتبار: {n[0]}",alert=True)
            if kind in {"audit_report","last_plan_changes","last_extensions","last_expiries","last_locks","last_exempts"}:
                rows=await (await db.execute("SELECT actor_id,target_id,action,detail,created_at FROM audit_logs WHERE action LIKE 'plan%' OR action LIKE 'user_backup_restore' ORDER BY id DESC LIMIT 20")).fetchall(); return await self.send_message(chat_id,"🧾 Audit اعتبار\n"+"\n".join(f"`{a}` → `{t}` · {act} · {det[:100]}" for a,t,act,det,ts in rows) if rows else "Audit اعتباری ثبت نشده است.")
        if kind == "self_test":
            required = ["global_enabled","default_days","plans"]; missing=[x for x in required if x not in cfg]; return await self.answer_callback(cq_id, "🧪 موتور اعتبار سالم ✅" if not missing else "❌ ناقص: "+", ".join(missing), alert=True)
        if kind == "sync_locks":
            from core.plan_control import enforce_expired_clients
            stopped=await enforce_expired_clients(ACTIVE_CLIENTS); return await self.answer_callback(cq_id,f"🔄 همگام‌سازی انجام شد؛ {stopped} سلف متوقف شد.",alert=True)
        if kind == "repair_missing_timers":
            from core.plan_control import apply_plan_expiry
            rows = await (await db.execute("SELECT user_id,COALESCE(plan,'normal') FROM users WHERE plan_started_at=0 OR plan_started_at IS NULL")).fetchall()
            for uid, pk in rows:
                await apply_plan_expiry(int(uid), str(pk), reset_start=True)
            return await self.answer_callback(cq_id, f"✅ {len(rows)} تایمر ناقص ساخته/اصلاح شد.", alert=True)
        if kind in {"repair_invalid_expiry","repair_locked_flags","clear_notice_flags"}:
            async with aiosqlite.connect(DB_NAME) as db:
                if kind=="repair_invalid_expiry": cur=await db.execute("UPDATE users SET plan_expires_at=0 WHERE plan_expires_at<0");
                elif kind=="repair_locked_flags": cur=await db.execute("UPDATE users SET plan_locked=0 WHERE plan_expires_at>0 AND plan_expires_at>? AND COALESCE(plan_exempt,0)=0",(now,));
                else: cur=await db.execute("UPDATE users SET plan_expired_notified=0 WHERE plan_expires_at>?",(now,))
                n=cur.rowcount; await db.commit()
            return await self.answer_callback(cq_id,f"🛠 {n} رکورد اصلاح شد.",alert=True)
        if kind == "show_config":
            return await self.send_message(chat_id, "```json\n"+json.dumps(cfg,ensure_ascii=False,indent=2)[:3800]+"\n```")
        return await self.answer_callback(cq_id, "ℹ️ گزارش آماده است.", alert=True)

    async def _execute_control_action(self, kind: str, cfg: dict[str, Any], chat_id: int, admin_id: int, cq_id: str):
        if kind == "toggle_audit":
            current = (await get_setting("plan_validity_audit", "1")) == "1"; await set_setting("plan_validity_audit", "0" if current else "1"); return await self.answer_callback(cq_id,f"🧾 Audit اعتبار {'فعال ✅' if not current else 'خاموش ❌'} شد.",alert=True)
        if kind == "audit_export":
            rows=[]
            async with aiosqlite.connect(DB_NAME) as db: rows=await (await db.execute("SELECT id,actor_id,target_id,action,detail,created_at FROM audit_logs WHERE action LIKE 'plan%' OR action='user_backup_restore' ORDER BY id DESC")).fetchall()
            path="downloads/plan_validity_audit.csv"; 
            ...
            await self.send_document(chat_id,path,"🧾 خروجی Audit اعتبار");
            try: os.remove(path)
            except OSError: pass
            return await self.answer_callback(cq_id, "✅ خروجی ارسال شد.", alert=True)
        if kind == "audit_cleanup":
            cutoff=int(time.time())-90*86400
            async with aiosqlite.connect(DB_NAME) as db: cur=await db.execute("DELETE FROM audit_logs WHERE created_at<? AND (action LIKE 'plan%' OR action='user_backup_restore')",(cutoff,)); await db.commit()
            return await self.answer_callback(cq_id,f"🧹 {cur.rowcount} رکورد قدیمی پاک شد.",alert=True)
        if kind == "reset_config":
            from core.plan_control import DEFAULT_CONFIG, save_plan_control
            import copy
            await save_plan_control(copy.deepcopy(DEFAULT_CONFIG)); return await self.answer_callback(cq_id, "♻️ پیکربندی اعتبار به پیش‌فرض برگشت.", alert=True)
        if kind == "snapshot_config":
            path="downloads/plan_validity_snapshot.json"; open(path,"w",encoding="utf-8").write(json.dumps(cfg,ensure_ascii=False,indent=2)); await self.send_document(chat_id,path,"📸 Snapshot اعتبار"); os.remove(path); return await self.answer_callback(cq_id, "✅ Snapshot ارسال شد.",alert=True)
        if kind == "restore_snapshot":
            USER_STATES[admin_id] = "APC_SNAPSHOT_UPLOAD"
            await self.answer_callback(cq_id, "📎 فایل Snapshot JSON را ارسال کنید.")
            return await self.send_message(chat_id, "📥 Snapshot معتبر تنظیمات اعتبار را به‌صورت JSON ارسال کنید.\n⚠️ فقط فایل SelfSaz پذیرفته می‌شود و قبل از ذخیره اعتبارسنجی می‌شود.", reply_markup={"inline_keyboard":[[{"text":"❌ لغو","callback_data":"recovery_cancel"}]]})
        if kind == "preview_expiry_notice":
            text=await get_setting("plan_expiry_notice","⏳ اعتبار پلن شما رو به پایان است."); return await self.send_message(chat_id,text)
        return await self.answer_callback(cq_id, "✅ انجام شد.", alert=True)
    ...
    def get_admin_hub_kb(self):
        return {"inline_keyboard": [
            ...
            [{"text": "⏳ ۲۰. اعتبار و انقضای پلن‌ها (۱۳۰ قابلیت)", "callback_data": "ad_sec_20"}],
    ...
    async def start(self):
        ...
        print("[+] Ultimate HTTP Bot Online with 175 Admin Features.", flush=True)

        async with aiohttp.ClientSession() as session:
            # This project uses long polling, not webhook mode.
            # A leftover webhook causes Telegram to return HTTP 409 for getUpdates.
            try:
                ...
                if webhook_url:
                    add_system_log(f"Webhook detected ({webhook_url}); removing it for long polling.")
                    async with session.post(
                        f"{API_URL}/deleteWebhook",
                        json={"drop_pending_updates": False},
                        timeout=aiohttp.ClientTimeout(total=15),
                    ) as resp:
                        deleted = await resp.json(content_type=None)
                    if not deleted.get("ok"):
                        add_system_log(f"deleteWebhook failed: {deleted.get('description', deleted)}")
                    else:
                        add_system_log("Webhook removed successfully; long polling is active.")
                else:
                    add_system_log("No webhook configured; long polling is active.")
            except Exception as exc:
                add_system_log(f"Telegram startup check failed: {type(exc).__name__}: {exc}")
                traceback.print_exc()

            try:
                me = await self.get_me()
                if me:
                    add_system_log(f"Telegram bot authenticated as @{me.get('username', me.get('first_name', 'unknown'))}.")
                else:
                    add_system_log("Telegram getMe failed or returned no bot identity. Check BOT_TOKEN.")
            except Exception as exc:
                add_system_log(f"Telegram getMe exception: {type(exc).__name__}: {exc}")
                ...
                try:
                    ...
                    async with session.get(url, timeout=aiohttp.ClientTimeout(total=25)) as resp:
                        ...
                        if resp.status != 200 or not data.get("ok"):
                            add_system_log(
                                f"Telegram getUpdates failed ({resp.status}): "
                        ...
                        for update in updates:
                            ...
                            add_system_log(
                                ...
                                f"type={'callback_query' if 'callback_query' in update else 'message' if 'message' in update else 'other'}"
                            )
                            asyncio.create_task(self._safe_handle_update(update))
                except asyncio.CancelledError:
                    raise
                except Exception as exc:
                    add_system_log(f"Polling exception: {type(exc).__name__}: {exc}")
                    traceback.print_exc()
                    await asyncio.sleep(3)
    ...
    async def render_user_dashboard(self, user_id: int, is_admin: bool = False):
        ...
        if not is_admin and state.get("expired"):
            text = (
                "🔒 **اعتبار پلن به پایان رسیده است**\n"
                "━━━━━━━━━━━━━━━━━━━━━\n"
                f"🏆 رنک فعلی: {plan_cfg['badge']} **{plan_cfg['title']}**\n"
                f"📅 پایان اعتبار: `{format_expiry(state.get('expires_at'))}`\n"
                f"🪙 سکه: `{(profile or {}).get('coins', u[1])}` | 👥 دعوت: `{(profile or {}).get('referral_count', 0)}`\n\n"
                "برای فعال‌شدن دوباره امکانات، فقط دعوت دوستان و ارتقای رنک در دسترس است. "
                "هر دکمه قدیمی مربوط به امکانات قفل‌شده نیز در سمت سرور اعتبارسنجی و رد می‌شود."
            )
            return text, self.get_expired_dashboard_kb()
        text = (
    ...
    async def _handle_admin_plan_input(self, action_id: int, text: str, chat_id: int, admin_id: int):
        catalog = build_admin_action_catalog()
        if action_id < 0 or action_id >= len(catalog):
            return await self.send_message(chat_id, "❌ عملیات اعتبار پیدا نشد.")
        item = catalog[action_id]; kind = item["kind"]; arg = item.get("arg")
        raw = text.strip()
        try:
            if kind == "global_int":
                value = int(raw); value = max(0, value)
                await set_plan_control_value(str(arg), value)
                await audit_log(admin_id, "plan_validity_setting", detail=f"{arg}={value}")
                return await self.send_message(chat_id, f"✅ مقدار `{arg}` روی `{value}` ذخیره شد.")
            if kind == "notify_days_input":
                values = sorted({max(0, int(x.strip())) for x in raw.split(",") if x.strip()}, reverse=True)[:10]
                await set_plan_control_value("notify_before_days", values)
                return await self.send_message(chat_id, "✅ روزهای اعلان ذخیره شد: " + ", ".join(map(str, values)))
            if kind == "plan_int":
                pk, field = arg; value = max(0, int(raw))
                await self._update_plan_rule(pk, field, value)
                await audit_log(admin_id, "plan_rule_setting", detail=f"{pk}.{field}={value}")
                return await self.send_message(chat_id, f"✅ {VALIDITY_PLAN_LABELS[pk]} / `{field}` = `{value}` ذخیره شد.")
            if kind in {"user_report","user_set_days","user_extend_days","user_expire_now","user_unexpire","user_exempt_toggle","user_reset_timer","user_set_expiry","user_lock_toggle","user_reapply_plan"}:
                parts = raw.split(maxsplit=1); uid = int(parts[0])
                async with aiosqlite.connect(DB_NAME) as db:
                    exists = await (await db.execute("SELECT plan,plan_expires_at,plan_exempt,plan_locked FROM users WHERE user_id=?", (uid,))).fetchone()
                if not exists: return await self.send_message(chat_id, "❌ کاربر پیدا نشد.")
                if kind == "user_report":
                    state = await user_plan_state(uid); return await self.send_message(chat_id, f"👤 `{uid}`\n🏆 پلن: {VALIDITY_PLAN_LABELS.get(state.get('plan'),state.get('plan'))}\n🟢 استثنا: {state.get('exempt')}\n🔒 قفل: {state.get('locked')}\n📅 انقضا: {format_expiry(state.get('expires_at'))}\n⏳ باقی‌مانده: {((state.get('remaining') or 0)/86400):.2f} روز")
                if kind == "user_set_days":
                    if len(parts) != 2: return await self.send_message(chat_id, "فرمت: `user_id days`")
                    days=int(parts[1]); now=int(time.time())
                    async with aiosqlite.connect(DB_NAME) as db:
                        await db.execute("UPDATE users SET plan_started_at=?,plan_expires_at=?,plan_locked=0 WHERE user_id=?",(now,now+max(0,days)*86400 if days else 0,uid)); await db.commit()
                elif kind == "user_extend_days":
                    if len(parts) != 2: return await self.send_message(chat_id, "فرمت: `user_id days`")
                    days=int(parts[1]); now=int(time.time())
                    async with aiosqlite.connect(DB_NAME) as db:
                        await db.execute("UPDATE users SET plan_expires_at=CASE WHEN plan_expires_at>? THEN plan_expires_at+? ELSE ? END,plan_locked=0 WHERE user_id=?",(now,max(0,days)*86400,now+max(0,days)*86400,uid)); await db.commit()
                elif kind == "user_expire_now":
                    async with aiosqlite.connect(DB_NAME) as db: await db.execute("UPDATE users SET plan_expires_at=?,plan_locked=1 WHERE user_id=?",(int(time.time())-1,uid)); await db.commit()
                    await stop_single_client(uid)
                elif kind == "user_unexpire":
                    from core.plan_control import apply_plan_expiry
                    await apply_plan_expiry(uid, str(exists[0] or "normal"), reset_start=True)
                    async with aiosqlite.connect(DB_NAME) as db: await db.execute("UPDATE users SET plan_locked=0 WHERE user_id=?",(uid,)); await db.commit()
                elif kind == "user_exempt_toggle":
                    new=0 if bool(exists[2]) else 1
                    async with aiosqlite.connect(DB_NAME) as db: await db.execute("UPDATE users SET plan_exempt=?,plan_locked=0 WHERE user_id=?",(new,uid)); await db.commit()
                elif kind == "user_reset_timer":
                    from core.plan_control import apply_plan_expiry
                    await apply_plan_expiry(uid, str(exists[0] or "normal"), reset_start=True)
                elif kind == "user_set_expiry":
                    if len(parts) != 2: return await self.send_message(chat_id, "فرمت: `user_id unix_timestamp`")
                    stamp=int(parts[1])
                    async with aiosqlite.connect(DB_NAME) as db: await db.execute("UPDATE users SET plan_expires_at=?,plan_started_at=?,plan_locked=0 WHERE user_id=?",(stamp,int(time.time()),uid)); await db.commit()
                elif kind == "user_lock_toggle":
                    new=0 if bool(exists[3]) else 1
                    async with aiosqlite.connect(DB_NAME) as db: await db.execute("UPDATE users SET plan_locked=? WHERE user_id=?",(new,uid)); await db.commit()
                    if new: await stop_single_client(uid)
                elif kind == "user_reapply_plan":
                    from core.plan_control import apply_plan_expiry
                    await apply_plan_expiry(uid, str(exists[0] or "normal"), reset_start=False)
                await audit_log(admin_id, "plan_validity_user", uid, detail=f"{kind}:{raw}")
                state = await user_plan_state(uid)
                return await self.send_message(chat_id, f"✅ عملیات `{kind}` روی `{uid}` انجام شد.\n📅 انقضا: `{format_expiry(state.get('expires_at'))}`\n🔒 قفل: `{state.get('locked')}`")
            if kind.startswith("broadcast_"):
                now=int(time.time()); cutoff = now + (86400*3 if kind=="broadcast_3d" else 86400 if kind=="broadcast_1d" else 0)
                async with aiosqlite.connect(DB_NAME) as db:
                    if kind=="broadcast_expired": rows=await (await db.execute("SELECT user_id FROM users WHERE plan_expires_at>0 AND plan_expires_at<=? AND COALESCE(plan_exempt,0)=0",(now,))).fetchall()
                    else:
                        lower=now; rows=await (await db.execute("SELECT user_id FROM users WHERE plan_expires_at>? AND plan_expires_at<=?",(lower,cutoff))).fetchall()
                sent=0
                for (uid,) in rows:
                    try:
                        res=await self.send_message(int(uid),raw)
                        if isinstance(res,dict) and res.get("ok"): sent+=1
                    except Exception: pass
                return await self.send_message(chat_id,f"✅ پیام برای `{sent}` کاربر ارسال شد.")
            if kind == "dm_user":
                parts=raw.split(maxsplit=1)
                if len(parts)!=2 or not parts[0].isdigit(): return await self.send_message(chat_id,"فرمت: `user_id متن`")
                res=await self.send_message(int(parts[0]),parts[1]); return await self.send_message(chat_id,"✅ پیام ارسال شد." if res.get("ok") else "❌ ارسال ناموفق بود.")
            notice_keys={"set_expiry_notice":"plan_expiry_notice","set_expired_notice":"plan_expired_notice","set_extension_notice":"plan_extension_notice","set_upgrade_notice":"plan_upgrade_notice","set_lock_notice":"plan_lock_notice"}
            if kind in notice_keys:
                await set_setting(notice_keys[kind], raw[:3000]); return await self.send_message(chat_id,"✅ متن ذخیره شد.")
        except Exception as exc:
            add_system_log(f"Admin plan input failed: {type(exc).__name__}: {exc}")
            return await self.send_message(chat_id, f"❌ مقدار نامعتبر است:\n`{str(exc)[:500]}`")
    ...
    async def handle_update(self, update):
        ...
        # Reload persistent global controls on every update so Railway restarts
        # do not silently revert admin settings.
        try:
            REGISTRATION_OPEN = (await get_setting("registration_open", "1")) == "1"
            GLOBAL_MAINTENANCE = (await get_setting("global_maintenance", "0")) == "1"
            ANTI_SPAM_PROTECT = (await get_setting("anti_spam_protect", "1")) == "1"
            LOG_DELETED_MSGS = (await get_setting("log_deleted_msgs", "1")) == "1"
            MAX_ALLOWED_SELFS = max(1, int(await get_setting("max_allowed_selfs", str(MAX_ALLOWED_SELFS))))
            CHANNEL_URL = await get_setting("support_channel_url", CHANNEL_URL)
        except Exception as exc:
            add_system_log(f"Global settings load failed: {type(exc).__name__}: {exc}")
        ...
        if "message" in update:
            ...
            # Persistent activity timestamp for real admin analytics.
            try:
                async with aiosqlite.connect(DB_NAME) as db:
                    await db.execute("UPDATE users SET last_seen_at=? WHERE user_id=?", (int(time.time()), user_id))
                    await db.commit()
            except Exception:
                pass
            ...
            if not is_admin:
                if not await self._membership_gate(user_id, chat_id):
                    return
                plan_state = await user_plan_state(user_id)
                if plan_state.get("expired") and not (text.startswith("/start") or text == "/panel") and not (text == "/status" and (await get_plan_control()).get("allow_status_expired", False)):
                    USER_STATES.pop(user_id, None)
                    return await self.send_message(chat_id, "🔒 اعتبار پلن شما تمام شده است. برای ادامه فقط از «رنک‌ها و ارتقا» یا «دعوت از دوستان» استفاده کنید.", reply_markup=self.get_expired_dashboard_kb())
            ...
            if msg.get("document") and USER_STATES.get(user_id, "").startswith("RECOVERY_WAIT:"):
                return await self._handle_recovery_document(chat_id, user_id, msg["document"])

            # Admin media/forward states are handled from the original message object
            # so photos, videos, documents and forwarded posts are real operations.
            if is_admin and USER_STATES.get(user_id) in {"AD_WAIT_BROADCAST_MEDIA", "AD_WAIT_FORWARD_ALL"}:
                state = USER_STATES.pop(user_id, None)
                targets_query = "SELECT user_id FROM users"
                async with aiosqlite.connect(DB_NAME) as db:
                    rows = await (await db.execute(targets_query)).fetchall()
                sent = 0
                for (target_uid,) in rows:
                    try:
                        res = await self.copy_message(int(target_uid), chat_id, int(msg.get("message_id")))
                        if res.get("ok"): sent += 1
                    except Exception as exc:
                        add_system_log(f"Admin media broadcast target {target_uid}: {exc}")
                    await asyncio.sleep(0.05)
                await audit_log(user_id, "broadcast_media" if state == "AD_WAIT_BROADCAST_MEDIA" else "forward_all", detail=str(sent))
                return await self.send_message(chat_id, f"✅ پیام رسانه‌ای/فوروارد برای `{sent}` کاربر ارسال شد.")

            if is_admin and USER_STATES.get(user_id) == "AD_WAIT_PIN_GLOBAL":
                state = USER_STATES.pop(user_id, None)
                parts = text.split()
                if len(parts) != 2 or not parts[0].lstrip("-").isdigit() or not parts[1].isdigit():
                    return await self.send_message(chat_id, "❌ فرمت صحیح: `chat_id message_id`")
                result = await self.pin_chat_message(int(parts[0]), int(parts[1]))
                await audit_log(user_id, "pin_global", detail=f"{parts[0]}:{parts[1]}")
                return await self.send_message(chat_id, "📌 پیام با موفقیت Pin شد." if result.get("ok") else f"❌ Pin ناموفق بود: {result.get('description','خطای نامشخص')}")
...
# دانلود یوتیوب: موتور مشترک در core/youtube.py قرار دارد.
                ...
                return
            if USER_STATES.get(user_id) == "WAITING_FIXED_BIO":
                USER_STATES.pop(user_id, None)
                value = text.strip()[:70]
                if not value:
                    return await self.send_message(chat_id, "❌ متن Bio نمی‌تواند خالی باشد.")
                await self.update_setting_db(user_id, "fixed_bio_text", value)
                if user_id in ACTIVE_CLIENTS:
                    ACTIVE_CLIENTS[user_id].fixed_bio_text = value
                    ACTIVE_CLIENTS[user_id].settings["fixed_bio_text"] = value
                    if getattr(ACTIVE_CLIENTS[user_id], "fixed_bio_enabled", False):
                        try: await ACTIVE_CLIENTS[user_id].update_profile(bio=value)
                        except Exception: pass
                return await self.send_message(chat_id, f"✅ متن Bio ثابت ذخیره شد:\n`{value}`")
            ...
            # کنترل‌های ورودی بخش اعتبار پلن‌ها
            if is_admin and isinstance(USER_STATES.get(user_id), str) and USER_STATES[user_id].startswith("APC_INPUT:"):
                state = USER_STATES.pop(user_id)
                action_id = int(state.split(":", 1)[1])
                return await self._handle_admin_plan_input(action_id, text, chat_id, user_id)

            # استیت‌های متنی ادمین
            if is_admin:
                if USER_STATES.get(user_id) == "AD_WAIT_BROADCAST_MEDIA":
                    # The original message object is required; this state is handled above.
                    return await self.send_message(chat_id, "📎 فایل/عکس/ویدیو را همین حالا ارسال کنید.")
                if USER_STATES.get(user_id) == "AD_WAIT_FORWARD_ALL":
                    return await self.send_message(chat_id, "🔁 پیام را Forward کنید تا برای همه کاربران ارسال شود.")
                ...
                if USER_STATES.get(user_id) == "AD_WAIT_BROADCAST":
                    ...
                    if text.isdigit():
                        ...
                        await set_setting("max_allowed_selfs", str(MAX_ALLOWED_SELFS))
                        await audit_log(user_id, "max_allowed_selfs_set", detail=str(MAX_ALLOWED_SELFS))
                    ...
                    USER_STATES.pop(user_id, None)
                    new_url = text.strip()
                    if not (new_url.startswith("https://t.me/") or new_url.startswith("http://t.me/") or new_url.startswith("https://telegram.me/")):
                        return await self.send_message(chat_id, "❌ لینک باید یک آدرس معتبر Telegram باشد.")
                    CHANNEL_URL = new_url
                    await set_setting("support_channel_url", new_url)
                    await audit_log(user_id, "support_channel_url_set", detail=new_url)
                    return await self.send_message(chat_id, f"📢 لینک کانال حامی به `{new_url}` تغییر یافت.")
...
# پردازش کلیک روی دکمه‌ها
            ...
            if not is_admin:
                plan_state = await user_plan_state(user_id)
                if plan_state.get("expired") and not expired_callback_allowed(data):
                    await self.answer_callback(cq["id"], "🔒 اعتبار پلن شما تمام شده است؛ فقط دعوت و ارتقای رنک فعال است.", alert=True)
                    return
            ...
            # ================== اتوماسیون واقعی حساب ==================
                ...
                text = (
                    ...
                    f"📌 Bio ثابت: {'فعال ✅' if getattr(cli, 'fixed_bio_enabled', False) else 'خاموش ❌'}\n"
                ...
                kb = {"inline_keyboard": [
                    ...
                    [{"text": "✍️ تنظیم متن Bio ثابت", "callback_data": "fixed_bio_text"}],
                    [{"text": "🟢 روشن/خاموش Bio ثابت", "callback_data": "fixed_bio_toggle"}],
            ...
            elif data == "fixed_bio_text":
                USER_STATES[user_id] = "WAITING_FIXED_BIO"
                current = (ACTIVE_CLIENTS.get(user_id).settings.get("fixed_bio_text") if user_id in ACTIVE_CLIENTS else "") or ""
                return await self.send_message(chat_id, f"✍️ متن Bio ثابت را ارسال کن.\nمتن فعلی: `{current[:250] if current else 'تنظیم نشده'}`")
            elif data == "fixed_bio_toggle":
                cli = ACTIVE_CLIENTS.get(user_id)
                if not cli:
                    return await self.answer_callback(cq["id"], "❌ ابتدا سلف را روشن کن.", alert=True)
                new = not bool(getattr(cli, "fixed_bio_enabled", False))
                try:
                    if new:
                        me = await cli.get_me()
                        if getattr(cli, "original_bio", None) is None:
                            cli.original_bio = getattr(me, "bio", "") or ""
                            await self.update_setting_db(user_id, "original_bio", cli.original_bio)
                        value = str(getattr(cli, "fixed_bio_text", "") or cli.settings.get("fixed_bio_text") or "SelfSaz • آنلاین 🟢")[:70]
                        if getattr(cli, "auto_bio_active", False):
                            cli.auto_bio_active = False
                            if getattr(cli, "auto_bio_task", None): cli.auto_bio_task.cancel(); cli.auto_bio_task = None
                            await self.update_setting_db(user_id, "automation_bio_active", False)
                        await cli.update_profile(bio=value); cli.fixed_bio_text=value
                    else:
                        await cli.update_profile(bio=(getattr(cli, "original_bio", "") or "")[:70])
                except Exception as exc:
                    return await self.answer_callback(cq["id"], f"❌ تغییر Bio ناموفق بود: {str(exc)[:220]}", alert=True)
                cli.fixed_bio_enabled = new
                cli.settings["fixed_bio_enabled"] = new
                cli.settings["fixed_bio_text"] = cli.fixed_bio_text
                await self.update_setting_db(user_id, "fixed_bio_enabled", new)
                return await self.answer_callback(cq["id"], f"📝 Bio ثابت {'روشن 🟢' if new else 'خاموش 🔴 و Bio قبلی بازیابی شد'}", alert=True)
                ...
                if data == "auto_name_toggle":
                    ...
                    new = not bool(getattr(cli, "auto_bio_active", False))
                    if new and getattr(cli, "fixed_bio_enabled", False):
                        return await self.answer_callback(cq["id"], "⚠️ Bio ثابت روشن است؛ ابتدا آن را خاموش کنید.", alert=True)
                    cli.auto_bio_active = new
            ...
            # ================== وضعیت اعتبار ==================
            elif data in {"plan_validity_status", "plan_validity_refresh"}:
                state = await user_plan_state(user_id)
                await self.answer_callback(cq["id"])
                text = (
                    "⏳ **وضعیت اعتبار شما**\n━━━━━━━━━━━━━━━━━━━━\n"
                    f"🏆 پلن: `{VALIDITY_PLAN_LABELS.get(state.get('plan'), state.get('plan'))}`\n"
                    f"📅 شروع: `{format_expiry(state.get('started_at'))}`\n"
                    f"📅 پایان: `{format_expiry(state.get('expires_at'))}`\n"
                    f"⏱ باقی‌مانده: `{((state.get('remaining') or 0)/86400):.2f}` روز\n"
                    f"🔒 وضعیت: `{'منقضی و قفل' if state.get('expired') else 'فعال'}`"
                )
                return await self.edit_message(chat_id, msg_id, text, reply_markup=self.get_expired_dashboard_kb() if state.get("expired") else self.get_main_dashboard_kb(user_id in ACTIVE_CLIENTS, user_id == ADMIN_ID))

            # ================== پشتیبان و بازیابی اطلاعات ==================
            elif data == "menu_recovery":
                await self.answer_callback(cq["id"])
                return await self.edit_message(chat_id, msg_id, "♻️ **مرکز پشتیبان و بازیابی کامل**\n━━━━━━━━━━━━━━━━━━━━\nبرای هر بخش خروجی مستقل یا فایل بازیابی مستقل داری. پشتیبان کامل تمام اطلاعات قابل‌بازیابی حساب را یکجا می‌گیرد.\n\n⚠️ Full Backup در صورت وجود شامل Session String نیز هست؛ آن را محرمانه نگه دار.", reply_markup=self.get_recovery_kb())
            elif data.startswith("recovery_export_"):
                scope = data.removeprefix("recovery_export_")
                if scope not in {"full","settings","relations","notes","automations","ai","stats","plan","notifications","session"}:
                    return await self.answer_callback(cq["id"], "❌ بخش نامعتبر است.", alert=True)
                try:
                    await self._send_user_backup(chat_id, user_id, scope)
                    return await self.answer_callback(cq["id"], "✅ فایل پشتیبان ارسال شد.", alert=True)
                except Exception as exc:
                    return await self.answer_callback(cq["id"], f"❌ خروجی ناموفق: {str(exc)[:250]}", alert=True)
            elif data.startswith("recovery_import_"):
                scope = data.removeprefix("recovery_import_")
                if scope not in {"full","settings","relations","notes","automations","ai","stats","plan","notifications","session"}:
                    return await self.answer_callback(cq["id"], "❌ بخش نامعتبر است.", alert=True)
                USER_STATES[user_id] = f"RECOVERY_WAIT:{scope}"
                await self.answer_callback(cq["id"], "📎 فایل JSON را ارسال کن.")
                return await self.send_message(chat_id, f"📥 فایل پشتیبان **{scope}** را به‌صورت JSON همین‌جا ارسال کن.\n\n✅ فقط فایل صادرشده توسط SelfSaz پذیرفته می‌شود.\n❌ User ID فایل باید با حساب فعلی یکی باشد.", reply_markup={"inline_keyboard":[[{"text":"❌ لغو", "callback_data":"recovery_cancel"}]]})
            elif data == "recovery_cancel":
                USER_STATES.pop(user_id, None)
                return await self.answer_callback(cq["id"], "بازیابی لغو شد.", alert=True)
            elif data == "recovery_validate":
                USER_STATES[user_id] = "RECOVERY_VALIDATE"
                await self.answer_callback(cq["id"], "📎 فایل JSON را برای اعتبارسنجی ارسال کن.")
                return await self.send_message(chat_id, "🔎 فایل پشتیبان JSON را بفرست؛ بدون بازیابی، فقط فرمت، نسخه و User ID بررسی می‌شود.")
            elif data == "recovery_security":
                return await self.answer_callback(cq["id"], "🔐 Full Backup ممکن است Session String داشته باشد؛ آن را در چت خصوصی نگه دار و برای دیگران نفرست.", alert=True)
            ...
            # ================== مرکز کاربری پیشرفته ==================
                ...
                kb={"inline_keyboard":[
                    ...
                    [{"text":"♻️ پشتیبان و بازیابی کامل اطلاعات","callback_data":"menu_recovery"}],
            ...
            # ساعت روی اسم — ۴۰ استایل حرفه‌ای متفاوت
            elif data == "menu_timename":
                from core.manager import FONTS, CLOCK_TEMPLATES
                cli = ACTIVE_CLIENTS.get(user_id)
                ...
                for i in range(1, 41, 2):
                    row=[]
                    for fid in (i, i+1):
                        digits = "".join(FONTS.get(fid, FONTS[1]).get(c,c) for c in "12:34")
                        preview = CLOCK_TEMPLATES.get(fid, "{t}").format(t=digits)
                        lock = "🔒 " if fid not in allowed else ("✅ " if fid == selected else "")
                        row.append({"text": f"{lock}استایل {fid} {preview}", "callback_data": f"font_{fid}"})
                    rows.append(row)
                rows.append([{"text": "ℹ️ نمایش ۴۰ استایل حرفه‌ای ساعت", "callback_data": "timename_info"}])
                rows.append([{"text": "🔙 بازگشت", "callback_data": "back_dashboard"}])
                await self.answer_callback(cq["id"])
                return await self.edit_message(chat_id, msg_id, f"⏰ **ساعت خودکار روی اسم (۴۰ استایل حرفه‌ای و متفاوت)**\nوضعیت فعلی: `{'روشن 🟢' if t_on else 'خاموش 🔴'}`\nاستایل انتخابی: `{selected or '۱'}`\nدسترسی پلن: `{len(allowed)}/40`", reply_markup={"inline_keyboard": rows})

            elif data == "timename_info":
                return await self.answer_callback(cq["id"], "🎨 ۴۰ استایل حرفه‌ای برای نمایش ساعت فعال است؛ استایل‌های قفل‌شده با ارتقای رنک باز می‌شوند.", alert=True)
                ...
                if f_id < 1 or f_id > 25:
                    return await self.answer_callback(cq["id"], "❌ فقط ۴۰ استایل رسمی ساعت وجود دارد.", alert=True)
...
# جعبه ابزارها
            ...
            elif data == "ad_sec_20" and is_admin:
                await self.answer_callback(cq["id"])
                return await self.edit_message(chat_id, msg_id, "⏳ **کنترل اعتبار و انقضای پلن‌ها — ۱۳۰ قابلیت واقعی**", reply_markup=self.get_admin_plan_validity_kb(1))
            elif data.startswith("ad_plan_validity_page_") and is_admin:
                page = int(data.rsplit("_", 1)[1])
                return await self.edit_message(chat_id, msg_id, f"⏳ **کنترل اعتبار پلن‌ها — صفحه {page}/13**", reply_markup=self.get_admin_plan_validity_kb(page))
            elif data == "ad_plan_validity_info" and is_admin:
                return await self.answer_callback(cq["id"], "📚 ۱۳۰ قابلیت مجزا در ۱۳ صفحه؛ تنظیمات پایدار و قابل Audit هستند.", alert=True)
            elif data.startswith("apc_") and is_admin:
                try:
                    action_id = int(data.removeprefix("apc_"))
                except ValueError:
                    return await self.answer_callback(cq["id"], "❌ شناسه قابلیت نامعتبر است.", alert=True)
                return await self._admin_plan_action(action_id, chat_id, user_id, msg_id, cq["id"])
            ...
            # قابلیت‌های دسته ۱
                ...
                for uid, cli in list(ACTIVE_CLIENTS.items()):
                    try:
                        connected = bool(getattr(cli, "is_connected", False))
                    except Exception:
                        connected = False
                    if not connected:
                        ACTIVE_CLIENTS.pop(uid, None)
                        cleaned += 1
                await audit_log(user_id, "admin_sync_connections", detail=str(cleaned))
                return await self.answer_callback(cq["id"], f"🔌 {cleaned} اتصال غیرفعال واقعاً از حافظه پاکسازی شد.", alert=True)
            ...
            # قابلیت‌های دسته ۲
                ...
                checked = invalid = 0
                # Validate at most 30 stored sessions per click to avoid blocking the bot.
                async with aiosqlite.connect(DB_NAME) as db:
                    rows = await (await db.execute("SELECT user_id,session_string FROM users WHERE session_string IS NOT NULL AND session_string!='' LIMIT 30")).fetchall()
                for target_uid, session_string in rows:
                    checked += 1
                    try:
                        cli = PyroClient(f"health_{target_uid}", api_id=API_ID, api_hash=API_HASH, session_string=session_string, in_memory=True)
                        await cli.connect()
                        await cli.disconnect()
                    except Exception as exc:
                        invalid += 1
                        try:
                            ...
                            pass
                        async with aiosqlite.connect(DB_NAME) as db:
                            await db.execute("UPDATE users SET session_string=NULL WHERE user_id=?", (target_uid,))
                            await db.commit()
                        await audit_log(user_id, "invalid_session_removed", target_id=int(target_uid), detail=str(exc)[:300])
                return await self.answer_callback(cq["id"], f"🩺 بررسی {checked} سشن انجام شد؛ {invalid} سشن نامعتبر پاک شد.", alert=True)
            ...
            # قابلیت‌های دسته ۶
                ...
                await set_setting("log_deleted_msgs", "1" if LOG_DELETED_MSGS else "0")
...
# قابلیت‌های دسته ۷
                ...
                USER_STATES[user_id] = "AD_WAIT_FORWARD_ALL"
                await self.answer_callback(cq["id"], "پیام را Forward کنید.")
                return await self.send_message(chat_id, "🔁 حالا همان پیام را Forward کنید تا برای همه کاربران ارسال شود.")
                ...
                USER_STATES[user_id] = "AD_WAIT_BROADCAST_MEDIA"
                await self.answer_callback(cq["id"], "رسانه را ارسال کنید.")
                return await self.send_message(chat_id, "📎 حالا عکس، ویدیو یا فایل را ارسال کنید تا برای همه کاربران کپی شود.")
                ...
                try:
                    async with aiosqlite.connect(DB_NAME) as db:
                        if action == "116":
                            ...
                            val=(await (await db.execute("SELECT COUNT(*) FROM users WHERE created_at>=?", (day_start,))).fetchone())[0]
                            seen=(await (await db.execute("SELECT COUNT(*) FROM users WHERE last_seen_at>=?", (day_start,))).fetchone())[0]
                            return await self.send_message(chat_id,f"👥 **گزارش واقعی کاربران امروز**\n🆕 ثبت‌نام امروز: `{val}`\n👀 فعال امروز: `{seen}`\n🟢 سلف‌های آنلاین: `{len(ACTIVE_CLIENTS)}`")
                        if action == "117":
                            cutoff=int(time.time())-7*86400
                            rows=await (await db.execute("SELECT COUNT(*) FROM users WHERE created_at>=?", (cutoff,))).fetchone()
                            return await self.send_message(chat_id,f"📈 کاربران ثبت‌شده در ۷ روز اخیر: `{rows[0]}`")
                        if action == "118":
                            now=int(time.time()); cutoff=now-30*86400
                            new=(await (await db.execute("SELECT COUNT(*) FROM users WHERE created_at>=?", (cutoff,))).fetchone())[0]
                            total=(await (await db.execute("SELECT COUNT(*) FROM users")).fetchone())[0]
                            return await self.answer_callback(cq["id"],f"📊 رشد ثبت‌نام ۳۰ روزه\n🆕 جدید: {new}\n👥 کل فعلی: {total}",alert=True)
                        if action == "119":
                        ...
                        if action == "121":
                            avg=(await (await db.execute("SELECT COALESCE(AVG(activity_score),0) FROM users")).fetchone())[0]
                            total=(await (await db.execute("SELECT COALESCE(SUM(activity_score),0) FROM users")).fetchone())[0]
                            return await self.answer_callback(cq["id"],f"⭐ میانگین XP: {avg:.1f}\n⚡ مجموع XP: {total}",alert=True)
                        if action == "122":
                        ...
                        if action == "123":
                            rows=await (await db.execute("SELECT user_id,last_seen_at FROM users ORDER BY last_seen_at DESC LIMIT 10")).fetchall()
                            return await self.send_message(chat_id,"⚡ **۱۰ کاربر فعال اخیر**\n"+"\n".join(f"• `{u}` — {datetime.fromtimestamp(int(ts or 0)).strftime('%Y-%m-%d %H:%M') if ts else 'نامشخص'}" for u,ts in rows) if rows else "هیچ داده‌ای نیست.")
                        if action == "124":
                        ...
                        if action=="147":
                            ...
                            n=(await (await db.execute("SELECT COUNT(*) FROM audit_logs WHERE created_at>=? AND (action LIKE '%upgrade%' OR action LIKE '%plan%')",(cutoff,))).fetchone())[0]
                            return await self.answer_callback(cq["id"],f"📈 رویدادهای ارتقا/پلن در ۳۰ روز اخیر: {n}",alert=True)
                        if action=="148":
                        ...
                        if action=="152":
                            ...
                            r=(await (await db.execute("SELECT COALESCE(SUM(activity_reward_coins),0) FROM users WHERE activity_reward_date=?",(today,))).fetchone())[0]
                            return await self.answer_callback(cq["id"],f"⚡ پاداش فعالیت ثبت‌شده امروز: {r}",alert=True)
                        if action=="153":
