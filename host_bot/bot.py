...
class HttpBot:
    ...
    async def ensure_registered_callback(self, cq: dict, user_id: int, chat_id: int, msg_id: int, data: str, is_admin: bool) -> bool:
        ...
        if is_admin or data in {"btn_create_session", "btn_cancel_login", "btn_submit_session", "btn_qr_refresh"}:
            return True
        u = await self.get_user_db(user_id)
        if u:
            return True
...
# پردازش کلیک روی دکمه‌ها
            ...
            # Only registration/re-authentication actions remain available after deletion.
