...
from core.plans import (
    ...
    process_referral, record_activity_reward, PLANS_DATA
)
...
class HttpBot:
    ...
    def get_main_dashboard_kb(self, is_online, is_admin=False):
        ...
        kb = [
            ...
            [{"text": "⏰ ساعت روی اسم (۱۰ فونت)", "callback_data": "menu_timename"}],
    ...
    def get_admin_sec18_kb(self):
        return self._admin_extra_section_kb(156, [
            ...
            ("پاکسازی کش AI 🧹", "adx_164"),("گزارش Smart Center 🧩", "adx_165"),
        ])
    ...
    async def handle_update(self, update):
        ...
        if "message" in update:
            ...
            if USER_STATES.get(user_id) == "WAITING_LOGIN_CODE":
                data = LOGIN_CLIENTS.get(user_id)
                if not data:
                    USER_STATES.pop(user_id, None)
...
# دانلود یوتیوب: موتور مشترک در core/youtube.py قرار دارد.
            ...
            if is_admin:
                if USER_STATES.get(user_id) == "AD_WAIT_BROADCAST":
                    ...
                    USER_STATES.pop(user_id, None)
                    if user_id in ACTIVE_CLIENTS:
                        await self.update_setting_db(user_id, "auto_reply_text", text[:3500])
                    return await self.send_message(chat_id, "✅ متن پاسخ خودکار شما ذخیره شد.")
...
# پردازش کلیک روی دکمه‌ها
            ...
            if data == "btn_resend_login_code":
                ...
                try:
                    ...
                    delivery = type(getattr(sent, "type", None)).__name__
                    note = "📱 کد با روش جایگزین ارسال شد. اکنون می‌توانی کد دریافتی را وارد کنی." if "App" not in delivery else "⚠️ هنوز کد داخل Telegram ارسال شده؛ آن را به ربات نفرست و QR را انتخاب کن."
                    buttons = []
                    if pending["can_resend"]:
                        buttons.append([{"text": "📩 روش بعدی", "callback_data": "btn_resend_login_code"}])
...
# جعبه ابزارها
            ...
            # ================== سوپر پنل مدیریت ادمین (۶۵ قابلیت مجزا) ==================
                ...
                await self.answer_callback(cq["id"])
                return await self.edit_message(chat_id, msg_id, "👑 **سوپر پنل مدیریت کل سیستم (۱۱۵ قابلیت تفکیک‌شده و فعال)**", reply_markup=self.get_admin_hub_kb())
                ...
                try:
                    async with aiosqlite.connect(DB_NAME) as db:
                        ...
                        if action in bool_keys:
                            key=bool_keys[action]; cur=await get_setting(key,"1"); new="0" if cur=="1" else "1"; await set_setting(key,new); await audit_log(user_id,f"toggle_{key}",detail=new); return await self.answer_callback(cq["id"],f"⚙️ {key}: {'فعال ✅' if new=='1' else 'خاموش ❌'}",alert=True)
                        ...
                        if action=="156": return await self.answer_callback(cq["id"],f"🤖 AI={'فعال' if (await get_setting('ai_global_enabled','1'))=='1' else 'خاموش'} | Smart={'فعال' if (await get_setting('smart_global_enabled','1'))=='1' else 'خاموش'}",alert=True)
                        if action=="157":
                        ...
                        if action=="159":
                            r=(await (await db.execute("SELECT COUNT(*) FROM users WHERE json_extract(settings,'$.monshi_active')=1")).fetchone())[0]; return await self.answer_callback(cq["id"],f"💬 منشی فعال: {r}",alert=True)
                        if action=="160":
                            r=(await (await db.execute("SELECT COUNT(*) FROM users WHERE json_extract(settings,'$.auto_reply_active')=1")).fetchone())[0]; return await self.answer_callback(cq["id"],f"🔁 پاسخ‌خودکار فعال: {r}",alert=True)
                        if action=="161":
                            r=(await (await db.execute("SELECT COUNT(*) FROM users WHERE json_extract(settings,'$.auto_read_active')=1")).fetchone())[0]; return await self.answer_callback(cq["id"],f"👁 Reader فعال: {r}",alert=True)
                        if action=="162":
