...
async def start_account_automations(client):
    ...
    client.auto_bio_active = bool(settings.get("automation_bio_active", False)) and not bool(settings.get("fixed_bio_enabled", False))
...
async def bio_on(client: Client, message: Message):
    if getattr(client, "fixed_bio_enabled", False):
        return await message.edit_text("❌ Bio ثابت روشن است؛ ابتدا Bio ثابت را خاموش کنید.")
